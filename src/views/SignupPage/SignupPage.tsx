...
import useSignupPageHook from './SignupPageHook';
...
const SignupPage = () => {
  const { register, handleSubmit, onSubmit, errors, isSubmitting } = useSignupPageHook();
  ...
  return (
    <section className="bg-accent-light-600 dark:bg-accent-dark-400 grid grid-cols-1 lg:grid-cols-2 w-screen h-screen p-4 gap-4 overflow-scroll font-archivo">
      <Helmet>
        ...
        <meta name="description" content="Welcome to the Signup Page" />
      </Helmet>
      ...
      <div className="w-full h-fit flex flex-col items-center self-center gap-2">
        <div className="w-3/4 grid gap-6">
          <div className="grid text-center">
            <h2 className="text-h4 font-bold text-text-light-600 dark:text-text-dark-600">{t('signup.mainTitle')}</h2>
            <p className="text-body4 text-text-dark-400">{t('signup.subTitle')}</p>
          </div>
          ...
          <form className="grid gap-2 w-full" onSubmit={handleSubmit(onSubmit)} noValidate>
            ...
            <FormField
              type="text"
              placeholder={t('form.homeTown.placeholder')}
              name="hometown"
              register={register}
              error={errors.hometown}
              label={t('form.homeTown.label')}
            />
          ...
          <span className="text-body4 text-text-light-600 dark:text-text-dark-500 text-center">
            ...
            <Link to={LOG_IN_ROUTE} className="text-primary-light-500 underline underline-offset-2">
              {t('signup.login')}
            </Link>
          </span>
        </div>
